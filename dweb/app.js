        ...
        .then((currentUser) => {
          ...
          User.findOne({ email: usuario })
            .then((user) => {
              ...
              console.log('Email de destino:', user.email);
              console.log('Email de origen:', emailSesion);
...
app.get('/p5_v2', verificarToken, (req, res) => {
  if (req.user) {
    ...
    const transferenciasFiltradas = transferencias.filter(({ correoOrigen, usuario }) => {
      return correoOrigen === emailSesion || usuario === emailSesion;
    });

    const transferenciasReducidas = transferenciasFiltradas.map(({ movimiento, monto, correoOrigen, glosa, usuario, fecha }) => {
      const esUsuarioActual = usuario === emailSesion;
      const movimientoActualizado = esUsuarioActual ? 'Transferencia recibida' : 'Transferencia a tercero';
      const usuarioActualizado = esUsuarioActual ? correoOrigen : usuario;
      return { movimiento: movimientoActualizado, monto, usuario: usuarioActualizado, correoOrigen, glosa, fecha };
    });
