...
pool.query('SELECT * FROM dweb', (err, result) => {
  if (err) {
...
app.post('/registro', (req, res) => {
  ...
  if (nombre === undefined || apellido === undefined || correo === undefined || password === undefined) {
    res.status(400).json({
  ...
  pool.query('SELECT * FROM dweb WHERE correo = $1', [correo], (err, result) => {
    if (err) {
      console.error('Error al ejecutar la consulta:', err);
      res.status(500).json({
    ...
    pool.query('INSERT INTO dweb (nombre, apellido, correo, password) VALUES ($1, $2, $3, $4)', [nombre, apellido, correo, password], (err, result) => {
      if (err) {
        console.error('Error al ejecutar la consulta:', err);
        res.status(500).json({
          message: 'Error al registrar el usuario'
      ...
      // Supongamos que la inserción fue exitosa y obtienes el ID del usuario y otros datos relacionados
      const userId = result.insertId; // Obtén el ID del usuario insertado
...
app.post('/login', (req, res) => {
  ...
  pool.query('SELECT * FROM dweb WHERE correo = $1 AND password = $2', [correo, password], (err, result) => {
    if (err) {
      console.error('Error al ejecutar la consulta:', err);
      res.status(500).json({
    ...
    // Las credenciales son válidas, generar un token JWT único para el inicio de sesión
